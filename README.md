...
マテリア精製と分解の操作支援、NPC販売場所の検索、外部マーケット最安値確認、日課チェックリスト、シャキ通知、キャラ選択画面の保存エモート再生を提供します。
...
- Stable: `0.3.7` / Dalamud API 15
- Testing: `0.3.7` / Dalamud API 15
...
- `/xivminiutil version` または `/xmu version` で、ゲーム内で読み込まれているDLLのパス、バージョン、更新時刻を確認できます。
...
3. 「Settings」タブの「Shop Search」から、安値3位までの表示とデータセンター外検索を切り替えられます。
...
- 「Checklist」タブで日課項目を完了にできます。通知時刻などは「Settings」タブの「Checklist」で設定します。
- 「Settings」タブの「シャキ通知」から通知音と再生時間を設定できます。
- 「Settings」タブの「キャラ選択 エモート」から、ログイン中に実行したエモートの記録とキャラ選択画面での再生を設定できます。
- キャラ選択背景の通常設定と1クリック確認は「Settings」タブの「ログイン背景」にあります。キャラ選択画面では、マウスで調整後に「現在の構図を保存」を押すと、次回の初期構図として再現されます。
