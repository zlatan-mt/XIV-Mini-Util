...
Test(403, "title background quickcheck idle 'Inactive' adapter after login is not treated as stale (substring false positive)", () =>
{
    // V2 は legacy camera adapter を意図的に arm しないため run 終了時の adapter.State は "Inactive"。
    // "Inactive" は substring "Active" に一致するが post-login leak ではない。Safe/Unsafe 判定を整合させる。
    // 実際の入力は enum の ToString（常に "Inactive"）。
    return TitleBackgroundQuickCheckEvaluator.IsSafeAfterLoginAdapterState("Inactive")
        && !TitleBackgroundQuickCheckEvaluator.IsUnsafeAfterLoginAdapterState("Inactive")
        && TitleBackgroundQuickCheckEvaluator.IsUnsafeAfterLoginAdapterState("Active")
        && !TitleBackgroundQuickCheckEvaluator.IsUnsafeAfterLoginAdapterState("SceneLoaded");
});

Test(406, "one-click run advances the quickcheck run-state to CharaSelectObserved for any new CharaSelect engine (V2 or placement proof)", () =>
{
    ...
    var body = ExtractMethodBody(hooks, "private void LobbySceneLoadedDetour(");
    // run-state 遷移は V2 だけでなく placement proof でも必要（owner=PlacementProof のとき IsV2Active は false）。
    var guardIndex = body.IndexOf("if (IsNewCharaSelectEngineActive)", StringComparison.Ordinal);
    var newEngineBlock = guardIndex >= 0 ? body.Substring(guardIndex) : string.Empty;
    var framingIdx = body.IndexOf("TryApplyV2Framing(", StringComparison.Ordinal);
    var v2FramingIdx = body.LastIndexOf("if (IsV2Active)", StringComparison.Ordinal);

    return body.Contains("ShouldHandleCharaSelectSceneReadySignal(", StringComparison.Ordinal)
        && guardIndex >= 0
        && newEngineBlock.Contains("_quickCheckState.RunState == TitleBackgroundQuickCheckRunState.Armed", StringComparison.Ordinal)
        && newEngineBlock.Contains("RunState = TitleBackgroundQuickCheckRunState.CharaSelectObserved", StringComparison.Ordinal)
        && newEngineBlock.Contains("!_clientState.IsLoggedIn", StringComparison.Ordinal)
        // framing 書込は V2 owner のときだけ（placement proof は camera を書かない）。
        && v2FramingIdx > guardIndex && framingIdx > v2FramingIdx;
});
