...
- **Title Background / Character Select を触る**: `projects/XIV-Mini-Util/Services/TitleBackground/AGENTS.md` と `/title-background` skill を先に読む。Sonnet/Codex へ委譲する subagent プロンプトを書く前、実機レポート判読前も同じ。
- **review 判断基準**: 共通ポリシーは `zlatan-mt/sol-review-bridge` の `docs/REVIEW_POLICY.md`（Web Sol へ渡る実体は同 repo の `prompts/review-policy.md`。隣接 checkout なら `../../sol-review-bridge/docs/REVIEW_POLICY.md`）。XIV 固有の差分は §7。
...
- 実機安全境界（安全ゲート、run-scoped 判定、pre-login snapshot、非永続 probe、`PersistentApplyEnabled`、world 座標を FixOn／カメラ焦点へ流さない）を自動化を理由に緩めない。詳細は Title Background nested `AGENTS.md`。
...
この節の詳細は `projects/XIV-Mini-Util/Services/TitleBackground/AGENTS.md` へ移設した。Title Background / Character Select（`Services/TitleBackground/`・`Services/CharaSelect/`・`Windows/Components/SettingsTab.TitleBackground.cs` / `.CharaSelect.cs`・`tools/CharaSelectLogicTests/`）を触るなら、その nested `AGENTS.md` と `/title-background` skill を先に読む。恒久契約は root の「実機確認は必要最小限」より優先する。要点だけ:
