...
public sealed partial class SettingsTab
{
    ...
    private void DrawTitleBackgroundSettings()
    {
        ...
        var previewLabel = TitleBackgroundCharaSelectPresetSelectionLogic.ResolvePreviewLabel(
            _configuration.TitleBackgroundOverrideEnabled,
            _configuration.TitleBackgroundCharacterSelectOverrideCandidateId);
        if (ImGui.BeginCombo("背景##TitleBackgroundPreset", previewLabel))
        {
            for (var i = 0; i < choices.Count; i++)
            {
                if (ImGui.Selectable(choices[i].DisplayLabel, i == selectedIndex))
                {
                    _titleBackgroundViewMessage = string.Empty;
                    var pickedId = TitleBackgroundCharaSelectPresetSelectionLogic.ResolveCandidateIdForIndex(i);
                    ...
                    {
                        _titleScreenBackgroundService.SelectTitleBackgroundPreset(pickedId);
                    }
                }
            }
        ...
        var presetSelected = _configuration.TitleBackgroundOverrideEnabled
            && TitleBackgroundCharaSelectPresetSelectionLogic.IsCuratedCandidateId(
                _configuration.TitleBackgroundCharacterSelectOverrideCandidateId);

        // 操作2: 主ボタン「この場所で確認を開始」（単一サービスメソッドだけを呼ぶ）。
        //        実行中／背景未選択(OFF)では無効。
        ImGui.Spacing();
        var oneClickDisabled = status.Busy || !presetSelected;
        if (oneClickDisabled)
        {
            ImGui.BeginDisabled();
        }
        ...
        // 操作3: キャラ選択中は現在の構図を保存し、それ以外では初期状態に戻す。
    ...
    private static string DescribeTitleBackgroundViewCaptureFailure(string status)
    {
        ...
        {
            "skipped-empty-candidate" => "先に背景を選んでください。",
