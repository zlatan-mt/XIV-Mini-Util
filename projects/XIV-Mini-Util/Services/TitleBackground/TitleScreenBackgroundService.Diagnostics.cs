...
public sealed unsafe partial class TitleScreenBackgroundService
{
    ...
    private Dictionary<string, string> BuildTransitionSnapshot(string eventName)
    {
        ...
        return new Dictionary<string, string>
        {
            ...
            ["runtimeRestoreSceneGeneration"] = _cameraRestoreCurve.LastCharaSelectCameraRuntimeRestoreSceneGeneration.ToString(),
        ...
        bool automaticInvocation = false)
    {
        ...
        var deliveryVerification = BuildTitleBackgroundVerificationSummary();
        // PR #7 cleanup: 新 CharaSelect placement engine が今回 run で確定
        // （resolve→capture→write→readback）した場合、legacy delivery 判定の top-line verdict を
        // その状態と整合させる。completed-run proof snapshot からのみ判定する（run-scoped。
        // runtime placement 挙動は不変で、判定は placementProofVerdict と同じ根拠を使う）。
        var deliveryCharaSelectPlacementEngineConfirmed = automaticInvocation
            && _automaticCheck.CompletedRunProof is { } completedPlacementProofForDelivery
            && (completedPlacementProofForDelivery.ConfirmedPlacementProof.Valid
                || (completedPlacementProofForDelivery.ApplyCount > 0
                    && completedPlacementProofForDelivery.WriteConfirmed));
        var phase2NDeliverySummary = TitleBackgroundDeliveryDiagnostic.BuildSummary(
            ...
            deliveryVerification.CharacterNumericallyVerified,
            deliveryCharaSelectPlacementEngineConfirmed);
        ...
        if (!includeDetailedPhase2Diagnostics)
        {
            ...
            [
                ...
                // TitleEdit-informed placement path の状態。auto-copy report が通る経路
                // （!includeDetailedPhase2Diagnostics）はここで return するため、この列挙へ入れる。
                $"v2.enabled={_configuration.TitleBackgroundV2Enabled}",
                $"v2.active={IsV2Active}",
                ..BuildCharaSelectPlacementDiagnosticLines(),
        ...
        var lines = new List<string>
        {
            ...
            // V2 production path (Il Mheg proof) の run-scoped 状態。legacy camera path が同時作動していない
            // ことは v2.legacyCameraPathInactive で読める（_savedViewPoseMaintain 非 active かつ Phase2G 未適用）。
            $"v2.enabled={_configuration.TitleBackgroundV2Enabled}",
            $"v2.active={IsV2Active}",
            $"v2.applied={_v2.FramingApplied}",
            $"v2.targetScene={FormatNone(_configuration.TitleBackgroundTerritoryPath)}",
            $"v2.sceneReadyObservedCount={_v2.SceneReadyObservedCount}",
            $"v2.framingAttemptCount={_v2.FramingAttemptCount}",
            $"v2.framingAppliedCount={_v2.FramingAppliedCount}",
            $"v2.boundedRetryRemaining={_v2.RetryRemaining}",
            $"v2.boundedSettleRemaining={_v2.SettleRemaining}",
            $"v2.windowClosed={_v2.WindowClosed}",
            $"v2.lastFramingStatus={FormatNone(_v2.LastFramingStatus)}",
            $"v2.lastStopReason={FormatNone(_v2.LastStopReason)}",
            $"v2.appliedPose=dirH={_v2.LastAppliedYaw:0.###};dirV={_v2.LastAppliedPitch:0.###};distance={_v2.LastAppliedDistance:0.###};fovY={_v2.LastAppliedFovY:0.###}",
            $"v2.appliedFrame={_v2.LastAppliedFrame}",
            $"v2.legacyCameraPathInactive={(!_savedViewPoseMaintain.Active && _phaseRecording.Phase2GGenerationOverrideSetMidAppliedCount == 0 && _phaseRecording.Phase2GGenerationOverrideLowHighAppliedCount == 0)}",
            $"v2.postLoginWritesStopped={(_v2.PostLoginWritesStopped || (IsV2Active && _clientState.IsLoggedIn && _v2.WindowClosed))}",
            $"v2.disposeState={(_hookLifecycle.Disposed ? "disposed" : "active")}",
            // TitleEdit-informed Character Select placement path の run-scoped 状態は
            // BuildCharaSelectPlacementDiagnosticLines() が単一ソースで出す（初期化子後に AddRange）。
            ...
            $"charaSelectCameraAdapter.sceneLoadedNotification={_cameraRestoreCurve.LastCharaSelectCameraRuntimeRestoreStatus}",
            $"charaSelectCameraAdapter.runtimeRecordStatus={_cameraRestoreCurve.LastCharaSelectCameraRuntimeRecordStatus}",
            $"charaSelectCameraAdapter.runtimeRecordFailureReason={FormatNone(_cameraRestoreCurve.LastCharaSelectCameraRuntimeRecordError)}",
            $"charaSelectCameraAdapter.runtimeRestoreFailureReason={FormatNone(_cameraRestoreCurve.LastCharaSelectCameraRuntimeRestoreFailureReason)}",
            $"charaSelectCameraAdapter.runtimeRestoreSceneGeneration={_cameraRestoreCurve.LastCharaSelectCameraRuntimeRestoreSceneGeneration}",
            ...
            $"sceneReadySignal.callCount={_cameraRestoreCurve.SceneReadySignalCallCount}",
            $"sceneReadySignal.rawCallCount={_cameraRestoreCurve.SceneReadySignalCallCount}",
            $"sceneReadySignal.acceptedCount={_cameraRestoreCurve.SceneReadySignalAcceptedCount}",
            $"sceneReadySignal.lastAdapterStateBeforeHandle={_cameraRestoreCurve.SceneReadySignalLastAdapterStateBeforeHandle}",
            $"sceneReadySignal.lastResolvedLobbyMap={_cameraRestoreCurve.SceneReadySignalLastResolvedLobbyMap}",
            $"runtimeRestore.attemptCount={_cameraRestoreCurve.RuntimeRestoreAttemptCount}",
            $"runtimeRestore.successCount={_cameraRestoreCurve.RuntimeRestoreSuccessCount}",
            $"runtimeRestore.lastStatus={_cameraRestoreCurve.LastCharaSelectCameraRuntimeRestoreStatus}",
            $"runtimeRestore.lastFailureReason={FormatNone(_cameraRestoreCurve.LastCharaSelectCameraRuntimeRestoreFailureReason)}",
            "runtimeRestore.target=LobbyCamera.DirH/DirV/Distance/InterpDistance/FoV",
            $"runtimeRestore.restoredYaw={FormatFloat(_cameraRestoreCurve.RuntimeRestoreLastRestoredYaw)}",
            $"runtimeRestore.restoredPitch={FormatFloat(_cameraRestoreCurve.RuntimeRestoreLastRestoredPitch)}",
            $"runtimeRestore.restoredDistance={FormatFloat(_cameraRestoreCurve.RuntimeRestoreLastRestoredDistance)}",
            $"runtimeRestore.restoredFovY={FormatFloat(_cameraRestoreCurve.RuntimeRestoreLastRestoredFovY)}",
            $"runtimeRestore.appliedFrame={FormatFrame(_cameraRestoreCurve.RuntimeRestoreAppliedFrame)}",
            $"cameraOneShotApply.status={_cameraRestoreCurve.LastCharaSelectCameraRuntimeRestoreStatus}",
            $"cameraOneShotApply.failureReason={FormatNone(_cameraRestoreCurve.LastCharaSelectCameraRuntimeRestoreFailureReason)}",
            $"cameraOneShotApply.appliedYaw={FormatFloat(_cameraRestoreCurve.RuntimeRestoreLastRestoredYaw)}",
            $"cameraOneShotApply.appliedPitch={FormatFloat(_cameraRestoreCurve.RuntimeRestoreLastRestoredPitch)}",
            $"cameraOneShotApply.appliedDistance={FormatFloat(_cameraRestoreCurve.RuntimeRestoreLastRestoredDistance)}",
            ...
            $"curveApply.attemptCount={_cameraRestoreCurve.CurveApplyAttemptCount}",
            $"curveApply.successCount={_cameraRestoreCurve.CurveApplySuccessCount}",
            $"curveApply.lastStatus={_cameraRestoreCurve.CurveApplyLastStatus}",
            $"curveApply.lastFailureReason={FormatNone(_cameraRestoreCurve.CurveApplyLastFailureReason)}",
            $"curveApply.appliedLow={FormatFloat(_cameraRestoreCurve.CurveApplyLastAppliedLow)}",
            $"curveApply.appliedMid={FormatFloat(_cameraRestoreCurve.CurveApplyLastAppliedMid)}",
            $"curveApply.appliedHigh={FormatFloat(_cameraRestoreCurve.CurveApplyLastAppliedHigh)}",
            ...
            $"curveApply.appliedFrame={FormatFrame(_cameraRestoreCurve.CurveApplyAppliedFrame)}",
            $"curveApply.requestedMid={FormatFloat(_cameraRestoreCurve.CurveApplyRequestedMid)}",
            $"curveApply.readBackValueImmediatelyAfterWrite={FormatUnavailable(_cameraRestoreCurve.CurveApplyReadBackValueImmediatelyAfterWrite, _cameraRestoreCurve.CurveApplyImmediateReadBackStatus)}",
            $"curveApply.activeCameraBefore.status={FormatNone(_cameraRestoreCurve.CurveApplyActiveCameraBeforeStatus)}",
            $"curveApply.activeCameraBefore.DirV={FormatFloat(_cameraRestoreCurve.CurveApplyActiveCameraBefore?.DirV)}",
            $"curveApply.activeCameraBefore.Distance={FormatFloat(_cameraRestoreCurve.CurveApplyActiveCameraBefore?.Distance)}",
            $"curveApply.activeCameraBefore.InterpDistance={FormatFloat(_cameraRestoreCurve.CurveApplyActiveCameraBefore?.InterpDistance)}",
            $"curveApply.activeCameraBefore.SceneCamera.LookAtVector={FormatVector(_cameraRestoreCurve.CurveApplyActiveCameraBefore?.LookAtVector)}",
            $"curveApply.activeCameraAfter.status={FormatNone(_cameraRestoreCurve.CurveApplyActiveCameraAfterStatus)}",
            $"curveApply.activeCameraAfter.DirV={FormatFloat(_cameraRestoreCurve.CurveApplyActiveCameraAfter?.DirV)}",
            $"curveApply.activeCameraAfter.Distance={FormatFloat(_cameraRestoreCurve.CurveApplyActiveCameraAfter?.Distance)}",
            $"curveApply.activeCameraAfter.InterpDistance={FormatFloat(_cameraRestoreCurve.CurveApplyActiveCameraAfter?.InterpDistance)}",
            $"curveApply.activeCameraAfter.SceneCamera.LookAtVector={FormatVector(_cameraRestoreCurve.CurveApplyActiveCameraAfter?.LookAtVector)}",
            $"curveApply.activeCameraImmediateDelta.DirV={FormatFloatDelta(_cameraRestoreCurve.CurveApplyActiveCameraAfter?.DirV, _cameraRestoreCurve.CurveApplyActiveCameraBefore?.DirV)}",
            $"curveApply.activeCameraImmediateDelta.LookAtVector={FormatVectorDelta(_cameraRestoreCurve.CurveApplyActiveCameraAfter?.LookAtVector, _cameraRestoreCurve.CurveApplyActiveCameraBefore?.LookAtVector)}",
            ...
            $"cameraCapture.lastStatus={_cameraRestoreCurve.LastCameraCaptureResult.Status}",
            $"cameraCapture.lastFailureReason={FormatNone(_cameraRestoreCurve.LastCameraCaptureResult.FailureReason)}",
            $"cameraCapture.lastFovState={_cameraRestoreCurve.LastCameraCaptureResult.FovState}",
            ...
            $"lastObservedFixOnCamera={FormatVector(_cameraObservation.LastObservedFixOnCamera)}",
            $"lastObservedFixOnFocus={FormatVector(_cameraObservation.LastObservedFixOnFocus)}",
            $"lastObservedFixOnFovY={(_cameraObservation.LastObservedFixOnFovY.HasValue ? _cameraObservation.LastObservedFixOnFovY.Value.ToString("0.###") : "none")}",
            $"lastFixOnFovY={(_cameraObservation.LastObservedFixOnFovY.HasValue ? _cameraObservation.LastObservedFixOnFovY.Value.ToString("0.###") : "none")}",
            $"lastCameraOverrideApplied={_cameraObservation.LastCameraOverrideApplied}",
            $"lastAppliedCamera={FormatVector(_cameraObservation.LastAppliedCamera)}",
            "cameraInputMeaning=FixOn camera argument; not yet verified as final stable camera position",
            $"lastAppliedFocus={FormatVector(_cameraObservation.LastAppliedFocus)}",
            "focusInputMeaning=FixOn focus argument; observed relation exists, but semantics remain under verification",
            $"lastAppliedFovY={(_cameraObservation.LastAppliedFovY.HasValue ? _cameraObservation.LastAppliedFovY.Value.ToString("0.###") : "none")}",
            $"lastFixOnInvocationMode={_cameraObservation.LastFixOnInvocationMode}",
            $"postFixOnCameraCaptureStatus={_cameraObservation.LastPostFixOnCameraCaptureStatus}",
            $"postFixOnCameraCaptureError={FormatNone(_cameraObservation.LastPostFixOnCameraCaptureError)}",
            $"postFixOnSceneCameraPosition={FormatVector(_cameraObservation.LastPostFixOnSceneCameraPosition)}",
            $"postFixOnLookAtVector={FormatVector(_cameraObservation.LastPostFixOnLookAtVector)}",
            ...
            $"postFixOnDistance={(_cameraObservation.LastPostFixOnDistance.HasValue ? _cameraObservation.LastPostFixOnDistance.Value.ToString("0.###") : "none")}",
            $"postFixOnFovY={(_cameraObservation.LastPostFixOnFovY.HasValue ? _cameraObservation.LastPostFixOnFovY.Value.ToString("0.###") : "none")}",
            ...
            $"cameraDelta.appliedToPostFixOn={FormatVectorDelta(_cameraObservation.LastPostFixOnSceneCameraPosition, _cameraObservation.LastAppliedCamera)}",
            $"cameraDelta.appliedToPostFixOn.x={FormatVectorAxisDelta(_cameraObservation.LastPostFixOnSceneCameraPosition, _cameraObservation.LastAppliedCamera, 0)}",
            $"cameraDelta.appliedToPostFixOn.y={FormatVectorAxisDelta(_cameraObservation.LastPostFixOnSceneCameraPosition, _cameraObservation.LastAppliedCamera, 1)}",
            $"cameraDelta.appliedToPostFixOn.z={FormatVectorAxisDelta(_cameraObservation.LastPostFixOnSceneCameraPosition, _cameraObservation.LastAppliedCamera, 2)}",
            $"cameraDelta.postFixOnToCurrent={FormatVectorDelta(currentSceneCameraPosition, _cameraObservation.LastPostFixOnSceneCameraPosition)}",
            $"cameraDelta.postFixOnToCurrent.x={FormatVectorAxisDelta(currentSceneCameraPosition, _cameraObservation.LastPostFixOnSceneCameraPosition, 0)}",
            $"cameraDelta.postFixOnToCurrent.y={FormatVectorAxisDelta(currentSceneCameraPosition, _cameraObservation.LastPostFixOnSceneCameraPosition, 1)}",
            $"cameraDelta.postFixOnToCurrent.z={FormatVectorAxisDelta(currentSceneCameraPosition, _cameraObservation.LastPostFixOnSceneCameraPosition, 2)}",
            $"focusDelta.appliedToPostFixOn={FormatVectorDelta(_cameraObservation.LastPostFixOnLookAtVector, _cameraObservation.LastAppliedFocus)}",
            $"focusDelta.appliedToPostFixOn.x={FormatVectorAxisDelta(_cameraObservation.LastPostFixOnLookAtVector, _cameraObservation.LastAppliedFocus, 0)}",
            $"focusDelta.appliedToPostFixOn.y={FormatVectorAxisDelta(_cameraObservation.LastPostFixOnLookAtVector, _cameraObservation.LastAppliedFocus, 1)}",
            $"focusDelta.appliedToPostFixOn.z={FormatVectorAxisDelta(_cameraObservation.LastPostFixOnLookAtVector, _cameraObservation.LastAppliedFocus, 2)}",
            $"focusDelta.postFixOnToCurrent={FormatVectorDelta(currentLookAtVector, _cameraObservation.LastPostFixOnLookAtVector)}",
            $"focusDelta.postFixOnToCurrent.x={FormatVectorAxisDelta(currentLookAtVector, _cameraObservation.LastPostFixOnLookAtVector, 0)}",
            $"focusDelta.postFixOnToCurrent.y={FormatVectorAxisDelta(currentLookAtVector, _cameraObservation.LastPostFixOnLookAtVector, 1)}",
            $"focusDelta.postFixOnToCurrent.z={FormatVectorAxisDelta(currentLookAtVector, _cameraObservation.LastPostFixOnLookAtVector, 2)}",
            $"currentVsPostFixOnCameraDelta={FormatVectorDelta(currentSceneCameraPosition, _cameraObservation.LastPostFixOnSceneCameraPosition)}",
            $"currentVsPostFixOnLookAtDelta={FormatVectorDelta(currentLookAtVector, _cameraObservation.LastPostFixOnLookAtVector)}",
            $"currentVsPostFixOnDistanceDelta={FormatFloatDelta(currentDistance, _cameraObservation.LastPostFixOnDistance)}",
            $"currentVsPostFixOnFovYDelta={FormatFloatDelta(currentFovY, _cameraObservation.LastPostFixOnFovY)}",
        ...
        lines.AddRange(BuildCharaSelectPlacementDiagnosticLines());
