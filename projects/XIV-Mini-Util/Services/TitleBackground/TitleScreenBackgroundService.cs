...
public sealed unsafe partial class TitleScreenBackgroundService : IDisposable
{
    ...
    // V2 production path (Il Mheg proof): scene-ready で bounded に 1 回だけ input params を書く run-scoped 状態。
    // V2 active 中は上の _savedViewPoseMaintain / Phase2G generated curve override は一切 arm しない（camera 制御排他）。
    private readonly TitleBackgroundV2RuntimeState _v2 = new();
    // TitleEdit-informed Character Select placement path (Phase 0 対応表 §6 / §7(A)) の run-scoped 状態。
    // active 中は legacy camera-maintain / 毎フレーム DrawObject placement / FixOn override を一切 arm しない
    // （V2 と同じ排他規約）。配置は (新 scene generation) または (選択キャラ変更) のときだけ bounded に書く。
    private readonly TitleBackgroundCharaSelectPlacementRuntimeState _charaSelectPlacement = new();
    // Elpis の active layout metadata と同一 terrain world position。run 中だけ保持し、
    // successful proof の既存 placement promotion へ値を渡す（native pointer は保持しない）。
    private readonly TitleBackgroundCharaSelectSourceLayoutRuntimeState _charaSelectSourceLayout = new();
    ...
    // Character Select エンジンの実 owner。OneClick run 中は _automaticCheck.PlacementProofArmed(runtime bool)
    // により PlacementProof になり、config 上の V2Enabled が True でも V2 は owner にならない
    // （PR #7 根本修正: config フリップ廃止）。排他判定はすべてこの owner から導く。
    private TitleBackgroundCharaSelectEngineOwner CharaSelectEngineOwner =>
        TitleBackgroundCharaSelectEngineOwnerLogic.Resolve(
            ...
            _configuration.TitleBackgroundCharaSelectPlacementEnabled,
            _configuration.TitleBackgroundV2Enabled);

    // V2 production path が実 owner か。proof armed 中は false（legacy とも V2 とも排他）。
    private bool IsV2Active =>
        TitleBackgroundCharaSelectEngineOwnerLogic.IsV2Owner(CharaSelectEngineOwner)
        && IsSourceBackedCandidateReadyForMutation();

    // TitleEdit-informed Character Select placement path（proof or persistent）が実 owner か。
    // true のとき legacy camera-maintain / 毎フレーム DrawObject placement / FixOn override は arm しない。
    private bool IsCharaSelectPlacementActive =>
        TitleBackgroundCharaSelectEngineOwnerLogic.IsPlacementOwner(CharaSelectEngineOwner);

    // 「新 CharaSelect エンジン」（V2 framing または TitleEdit-informed placement）のいずれかが owner か。
    // legacy ownership 排他の single source of truth。どちらか一方だけが character/camera state を書く。
    private bool IsNewCharaSelectEngineActive =>
        TitleBackgroundCharaSelectEngineOwnerLogic.IsNewEngineOwner(CharaSelectEngineOwner);
    ...
    public void SetEnabled(bool enabled)
    {
        ...
        if (!enabled)
        {
            // OFF では V2 / placement path フラグも落とす（通常画面 OFF = 完全無効の恒久契約）。
    ...
    // 通常画面の curated 背景セレクタ用の単一入口。OFF 相当（curated 以外）が来たら機能を無効化する。
    internal TitleBackgroundSimpleUiSummary SelectTitleBackgroundPreset(string? candidateId)
    {
        if (!TitleBackgroundCharaSelectPresetSelectionLogic.IsCuratedCandidateId(candidateId))
        {
            SetEnabled(false);
            return TitleBackgroundQuickCheckUiPresenter.BuildSimpleSummary(_configuration);
        }

        ApplySimpleAutoSetup(candidateId);
        return TitleBackgroundQuickCheckUiPresenter.BuildSimpleSummary(_configuration);
    }

    private void ApplySimpleAutoSetup()
    {
        ApplySimpleAutoSetup(null);
    }

    // candidateId=null なら config 上の選択中 curated candidate（無ければ既定 Il Mheg）を適用する。
    private void ApplySimpleAutoSetup(string? candidateId)
    {
        var previousCandidateId = TitleBackgroundCharacterSelectOverrideCandidateRegistry.NormalizeId(
        ...
        TitleBackgroundQuickCheckUiPresenter.ApplySimpleAutoSetup(_configuration, candidateId);
        var selectedCandidateId = TitleBackgroundCharacterSelectOverrideCandidateRegistry.NormalizeId(
            _configuration.TitleBackgroundCharacterSelectOverrideCandidateId);
        if (!string.Equals(previousCandidateId, selectedCandidateId, StringComparison.Ordinal))
        {
            // 候補変更は、旧候補の source-backed layout/position を再利用しない。
            _charaSelectSourceLayout.Reset();
            _charaSelectStaticAnchor.Reset();
            _charaSelectSceneObjectSuppression.Reset();
        }
    ...
    private void RestoreCharaSelectRuntimeCameraStateAfterSceneLoad()
    {
        ...
        var restoreDecision = TitleBackgroundFixOnViewOverrideLogic.ResolveRuntimeCameraRestoreDecision(
        ...
        if (restoreDecision == TitleBackgroundFixOnViewOverrideLogic.RestoreDecisionYieldNoPose)
        {
            _cameraRestoreCurve.LastCharaSelectCameraRuntimeRestoreStatus = "yielded-to-saved-view-no-pose";
            _cameraRestoreCurve.LastCharaSelectCameraRuntimeRestoreFailureReason = string.Empty;
            _cameraRestoreCurve.SavedViewPoseLastRestoreStatus = "yielded-to-saved-view-no-pose";
            _cameraRestoreCurve.SavedViewPoseLastRestoreSceneGeneration = _charaSelectCameraAdapter.RuntimeState.SceneGeneration;
            RecordTransitionEvent("runtime restore failed", "yielded-to-saved-view-no-pose");
            return;
        }
    ...
    private bool ApplySavedViewCameraPoseAfterFixOn(TitleBackgroundCharaSelectView savedView)
    {
        ...
        {
            _cameraRestoreCurve.SavedViewPoseLastRestoreStatus = "disabled-by-source-gate";
            return false;
        }

        // V2 production path 有効時は legacy saved-view pose 経路を無効化する（camera 制御排他）。
        if (!TitleBackgroundV2Logic.IsLegacyCameraMaintenanceAllowed(IsNewCharaSelectEngineActive))
        {
            _cameraRestoreCurve.SavedViewPoseLastRestoreStatus = "disabled-by-v2";
            return false;
        }
        ...
        _cameraRestoreCurve.SavedViewPoseAppliedDirH = savedView.DirH;
        ...
        ResetFacingVerificationForSavedViewLoad();
    ...
    private bool IsSavedViewSuppressedByAutomaticRun()
    {
        return _automaticCheck.Requested;
    }
        ...
        out string errorMessage)
    {
        ...
        return TryApplyLobbyCameraPose(
            restoredYaw.Value,
            runtimeState.Pitch.Value,
            runtimeState.Distance.Value,
        ...
        out string errorMessage)
    {
        ...
        try
        {
            ...
            lobbyCamera->DirH = dirH;
            lobbyCamera->DirV = dirV;
            lobbyCamera->Distance = distance;
            lobbyCamera->InterpDistance = distance;
            lobbyCamera->FoV = fovY;
            return true;
        }
        catch (Exception ex)
        {
    ...
    {
        if (!IsOverrideMutationBranchArmed()
        ...
        return _charaSelectSourceLayout.IsUsableFor(candidate)
            || (_configuration.TitleBackgroundCharaSelectPlacementEnabled
                && string.Equals(
    ...
    // Source-backed candidate は live active-layout と同一 terrain の Position が確定するまで、
    // scene/camera/character の全 writer を止める。既存の static candidate は従来どおり通す。
    private bool IsSourceBackedCandidateReadyForMutation()
    {
        var candidate = ResolveCurrentOverrideCandidate();
        if (!candidate.RequiresSourceBackedLayout)
        {
        ...
        if (_charaSelectSourceLayout.IsUsableFor(candidate))
        {
            return true;
        }

        return _configuration.TitleBackgroundCharaSelectPlacementEnabled
            && string.Equals(
                ...
                StringComparison.Ordinal)
            && _configuration.TitleBackgroundCharaSelectPlacementPositionCaptured
            && TitleBackgroundCameraMath.IsFiniteVector(new Vector3(
            ...
            && float.IsFinite(_configuration.TitleBackgroundCharaSelectPlacementRotation)
            && TitleBackgroundCharaSelectSourceLayoutLogic.IsCandidateFieldsValid(
                candidate,
                _configuration.TitleBackgroundTerritoryPath,
    ...
    private string GetFixOnFocusOverrideContextReason()
    {
        ...
        if (!IsSourceBackedCandidateReadyForMutation())
        {
            return "source-backed-candidate-not-ready";
        }
    ...
    private void ResetFixOnExperimentSnapshot()
    {
        ...
        // 前回の view 関与 load の trace は次の自動確認レポートまで保持する。収集中ならここで
        // 読み取りだけ停止する。ただしadapter reset後のgeneration再採番と衝突したstale traceは破棄する。
        _viewReplayTrace.PrepareForSceneLoad(
            _activeCharaSelectSceneGeneration,
            preserveSameGenerationTraceForSuppressedRun: _automaticCheck.Requested);
    }
    ...
    private void NormalizeConfiguration()
    {
        ...
        // The Old Sharlayan candidate is retained in the registry for compatibility, but its
        // previous runtime path is deferred. A stale enabled config must therefore fail closed
        // while keeping the candidate id available for the user to replace later.
        if (_configuration.TitleBackgroundOverrideEnabled
            && string.Equals(
                TitleBackgroundCharacterSelectOverrideCandidateRegistry.NormalizeId(
                    _configuration.TitleBackgroundCharacterSelectOverrideCandidateId),
                DeferredOldSharlayanCandidateId,
                StringComparison.Ordinal))
        {
            _configuration.TitleBackgroundOverrideEnabled = false;
            _configuration.TitleBackgroundV2Enabled = false;
    ...
    {
        // 新 CharaSelect engine（V2 / persistent placement / placement proof）のいずれかが
        // owner のときは legacy camera adapter を絶対に arm しない。placement を V2 と
        // 別扱いにすると saved-view / curve writer が同時に動くため、owner を単一遮断点にする。
        var enabled = !IsNewCharaSelectEngineActive
    ...
    private void EndCharaSelectTitleBackgroundSession(string reason, string source)
    {
        ...
        // V2 の bounded framing ウィンドウも session 終了で確実に閉じる（login/leaving 両方）。
        _v2.CloseWindow(reason);
    ...
    private void AddCharacterPlacementPreLoginCaptureLines(List<string> lines)
    {
        ...
        lines.Add($"character.facing.calibration.source={FormatNone(_characterPlacement.FacingCalibrationSource)}");
        lines.Add($"character.facing.calibration.offset={(_characterPlacement.FacingCalibrationOffset.HasValue ? _characterPlacement.FacingCalibrationOffset.Value.ToString("0.###") : "none")}");
        lines.Add($"character.facing.calibration.derivedOffset={(_characterPlacement.FacingCalibrationDerivedOffset.HasValue ? _characterPlacement.FacingCalibrationDerivedOffset.Value.ToString("0.###") : "none")}");
        lines.Add($"character.facing.calibration.naturalRotation={(_characterPlacement.FacingCalibrationNaturalRotation.HasValue ? _characterPlacement.FacingCalibrationNaturalRotation.Value.ToString("0.###") : "none")}");
        lines.Add($"character.facing.calibration.expectedFromGeometry={(_characterPlacement.FacingCalibrationExpectedFromGeometry.HasValue ? _characterPlacement.FacingCalibrationExpectedFromGeometry.Value.ToString("0.###") : "none")}");
        lines.Add($"character.facing.calibration.sampleCount={_characterPlacement.FacingCalibrationSampleCount}");
        lines.Add($"character.facing.calibration.stableSampleCount={_characterPlacement.FacingCalibrationSampleCount}");
        lines.Add($"character.facing.calibration.rejectedTransientCount={_characterPlacement.FacingCalibrationRejectedTransientCount}");
        lines.Add($"character.facing.calibration.maxOffsetDelta={(_characterPlacement.FacingCalibrationMaxOffsetDelta.HasValue ? _characterPlacement.FacingCalibrationMaxOffsetDelta.Value.ToString("0.###") : "none")}");
        lines.Add($"character.facing.geometricExpectedYaw={(_characterPlacement.FacingGeometricExpectedYaw.HasValue ? _characterPlacement.FacingGeometricExpectedYaw.Value.ToString("0.###") : "none")}");
        lines.Add($"character.facing.angularError={(_characterPlacement.FacingAngularError.HasValue ? _characterPlacement.FacingAngularError.Value.ToString("0.###") : "none")}");
        lines.Add($"character.facing.maxAngularError={(_characterPlacement.FacingMaxAngularError.HasValue ? _characterPlacement.FacingMaxAngularError.Value.ToString("0.###") : "none")}");
        lines.Add($"character.facing.settledConsecutiveFrameCount={_characterPlacement.FacingSettledConsecutiveFrameCount}");
        lines.Add($"character.facing.settledMaxAngularError={(_characterPlacement.FacingSettledMaxAngularError.HasValue ? _characterPlacement.FacingSettledMaxAngularError.Value.ToString("0.###") : "none")}");
        lines.Add($"character.facing.offsetAbsError={(_characterPlacement.FacingOffsetAbsError.HasValue ? _characterPlacement.FacingOffsetAbsError.Value.ToString("0.###") : "none")}");
        lines.Add($"character.facing.maxFrameDelta={(_characterPlacement.FacingMaxFrameDelta.HasValue ? _characterPlacement.FacingMaxFrameDelta.Value.ToString("0.###") : "none")}");
        lines.Add($"character.facing.maxAppliedError={(_characterPlacement.FacingMaxAppliedError.HasValue ? _characterPlacement.FacingMaxAppliedError.Value.ToString("0.###") : "none")}");
        lines.Add($"character.facing.lastError={FormatNone(_characterPlacement.FacingLastError)}");
        AddTitleBackgroundVerificationLines(lines);
        ...
        var weatherCandidate = ResolveCurrentOverrideCandidate();
        var requestedWeatherId = TitleBackgroundEnvironmentWeatherPolicy.ResolveRequestedWeatherId(weatherCandidate.Id);
        var appliedWeatherId = string.Equals(
            _environmentClearSky.LastCandidateId,
            weatherCandidate.Id,
            StringComparison.Ordinal)
            ? _environmentClearSky.LastAppliedWeatherId
            : null;
        ...
        lines.Add($"environment.weatherCandidate={weatherCandidate.Id}");
        lines.Add($"environment.weatherRequestedId={requestedWeatherId}");
        lines.Add($"environment.weatherAppliedId={appliedWeatherId?.ToString() ?? "none"}");
        ...
        // 背景セッション中の環境正午上書き（ログイン画面の暗さ対策）。config値・適用回数・最終status。
        ...
        // candidate-specific 時刻ポリシー（FRU のみ非正午。白飛び対策）。
        lines.Add($"environment.timePolicyName={FormatNone(_environmentNoon.LastPolicyName)}");
        lines.Add($"environment.dayTimeRequestedSeconds={FormatFloat(_environmentNoon.LastRequestedDayTimeSeconds)}");
        // pre-login で凍結した FRU environment snapshot。report 生成時の post-login / current-world
        // live 読取（environment.weather 等）と混同しないための値。
        lines.Add($"environment.preLogin.snapshotCaptured={_environmentClearSky.PreLoginSnapshotCaptured}");
        lines.Add($"environment.preLogin.weatherRequestedId={(_environmentClearSky.LastRequestedWeatherId?.ToString() ?? "none")}");
        lines.Add($"environment.preLogin.weatherAppliedId={(_environmentClearSky.LastAppliedWeatherId?.ToString() ?? "none")}");
        lines.Add($"environment.preLogin.weatherReadbackId={(_environmentClearSky.LastPreLoginWeatherReadback?.ToString() ?? "none")}");
        lines.Add($"environment.preLogin.timePolicyName={FormatNone(_environmentNoon.LastPolicyName)}");
        lines.Add($"environment.preLogin.dayTimeRequestedSeconds={FormatFloat(_environmentNoon.LastRequestedDayTimeSeconds)}");
        lines.Add($"environment.preLogin.dayTimeReadbackSeconds={(_environmentNoon.LastPreLoginDayTimeSecondsReadback.HasValue ? FormatFloat(_environmentNoon.LastPreLoginDayTimeSecondsReadback.Value) : "none")}");
        ...
        // 保存 view のネイティブ pose（DirH/DirV/Distance）。poseCaptured=False は旧形式の保存 view で、
        // pose 復元は行われない（再保存すると pose 付きへ昇格する）。
        lines.Add($"view.poseCaptured={_configuration.TitleBackgroundCharaSelectViewPoseCaptured}");
        ...
        lines.Add($"view.poseAppliedDirV={(_cameraRestoreCurve.SavedViewPoseAppliedDirV.HasValue ? _cameraRestoreCurve.SavedViewPoseAppliedDirV.Value.ToString("0.###") : "none")}");
        lines.Add($"view.poseAppliedDistance={(_cameraRestoreCurve.SavedViewPoseAppliedDistance.HasValue ? _cameraRestoreCurve.SavedViewPoseAppliedDistance.Value.ToString("0.###") : "none")}");
        ...
        // 保存view再現バグ診断（read-only）: FixOn view override適用直後〜後続フレームのSceneCamera実値trace。
        // 「FixOn引数と最終表示がそもそも一致するか」「一致するとして後から何が上書きするか」を1回の
        // 自動確認runだけで切り分けるための診断行（view.trace.*）。
        AddViewReplayTraceLines(lines);
