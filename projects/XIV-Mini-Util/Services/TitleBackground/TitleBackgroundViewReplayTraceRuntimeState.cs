...
internal sealed class TitleBackgroundViewReplayTraceRuntimeState
{
    ...
    public void PrepareForSceneLoad(
        ...
        bool preserveSameGenerationTraceForSuppressedRun = false)
    {
        // adapter reset後はgenerationが0から再採番される。同じ番号のtraceがload開始時点で残っていれば
        // 同一loadではなく旧epochとの衝突なので通常は破棄する。自動runはview適用を全経路で抑止し、
        // 新traceと混ざらないため、直前の通常表示を検証する保持traceだけは例外的に残す。
        if (TraceSceneGeneration != 0
            && TraceSceneGeneration == activeSceneGeneration
            && !preserveSameGenerationTraceForSuppressedRun)
        {
            Reset();
            return;
        }
