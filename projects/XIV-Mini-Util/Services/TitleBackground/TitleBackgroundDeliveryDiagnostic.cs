...
internal static class TitleBackgroundDeliveryDiagnostic
{
    ...
    public static TitleBackgroundDeliverySummary BuildSummary(
        ...
        bool characterNumericallyVerified = false,
        // PR #7: 新 CharaSelect placement engine が今回 run で resolve→capture→write→readback を
        // 確定した場合の signal。legacy delivery 判定は旧 per-frame path 前提で、これが false のままだと
        // top-line verdict が working-background-only に留まり placementProofVerdict=PASS と矛盾する。
        // true のときは既にテスト済みの character-composition 成立経路（safeNumericComposition）を通す。
        bool charaSelectPlacementEngineConfirmed = false)
    {
        ...
        // 旧 per-frame numeric 検証 か 新 placement engine 確定 のどちらかで character composition 成立。
        var characterCompositionVerified = characterNumericallyVerified
            || charaSelectPlacementEngineConfirmed;
        var safeNumericComposition = characterCompositionVerified
            && lastOverrideApplied
            && !string.Equals(transitionSafety, "unsafe", StringComparison.Ordinal)
        ...
        var characterObserved = safeNumericComposition
            ...
            : BuildCharacterVisibilityObserved(
                ...
                drawObjectNonNullCount,
                modelLikeNonNullCount);
        var characterBlocker = safeNumericComposition
            ? "none"
            : characterCompositedApplied
        ...
        var selectedVerdict = SelectDeliveryVerdict(
            backgroundMode,
            transitionSafety,
            lastOverrideApplied,
            nativeResolution,
            ...
            presetCompatibility,
            lighting);
        var verdict = safeNumericComposition
            ? "working-character-composition"
            : selectedVerdict.Verdict;
        var nextAction = safeNumericComposition
            ? "none"
            : selectedVerdict.NextAction;
        var nextActionReason = safeNumericComposition
            ? "numeric composition verification passed"
        ...
        var userMessage = safeNumericComposition
            ? "Background, character framing, and facing passed numeric verification."
            : characterCompositedApplied
        ...
        var userNextAction = safeNumericComposition
            ? "No visual confirmation is required."
            : nativeResolution == "found-single"
        ...
        var (mvpStatus, mvpBlockingIssue, mvpKnownLimitation) = safeNumericComposition
            ? ("complete-character-composition", "none", "none")
            : characterCompositedApplied
