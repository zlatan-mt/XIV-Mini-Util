...
{
    // FFXIV の Weather シート row 1 = "Clear Skies"（晴れ）、row 2 = "Fair Skies"（晴天）。
    // EnvManager.ActiveWeather は
    ...
    public const byte FairSkiesWeatherId = 2;
    ...
    public static bool TryApplyFairSky()
    {
        var manager = EnvManager.Instance();
        if (manager == null)
        {
        ...
        manager->ActiveWeather = FairSkiesWeatherId;
        return true;
    }
}
