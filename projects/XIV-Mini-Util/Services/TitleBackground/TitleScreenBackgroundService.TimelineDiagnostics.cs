...
public sealed unsafe partial class TitleScreenBackgroundService
{
    ...
    private void CaptureCharacterPlacementPlacementFrame(int frame, string reason)
    {
        ...
        if (TryResolveCharaSelectActorContext(out var legacyAimActor)
            && TitleBackgroundCharacterSourceProbe.TryReadCharaSelectCharacterAim(legacyAimActor, out var drawPosition, out var drawRotation))
        {
    ...
    private void MaintainCharaSelectCharacterPlacement()
    {
        try
        {
            // 新 CharaSelect エンジン（V2 framing または TitleEdit-informed placement）active 中は
            // legacy の毎フレーム DrawObject placement を止める（排他）。placement path は
            // MaintainTitleEditInformedCharaSelectPlacement が event-driven / bounded に配置する。
            ...
            // legacy 経路も canonical resolver のみを通す。unresolved なら write/read しない（fail-closed）。
            if (!TryResolveCharaSelectActorContext(out var placementActor))
            {
                MarkCharacterFacingInactive("character-unresolved");
                _characterPlacement.CharaSelectCharacterPlacementLastError = "character-unresolved";
                return;
            }

            if (TitleBackgroundCharacterSourceProbe.TrySetCurrentCharacterDrawPosition(placementActor, target))
            {
                ...
                CaptureFacingCalibrationDuringAutomaticRun(
                    placementActor,
                    activeCandidate,
                    target,
                    camera.SceneCameraPosition);
                ApplySavedViewCharacterFacing(
                    placementActor,
                    activeCandidate,
                    target,
                    camera.SceneCameraPosition);
            ...
            {
                MarkCharacterFacingInactive("draw-position-write-failed");
    ...
    private void CaptureFacingCalibrationDuringAutomaticRun(
        in CharaSelectResolvedActorContext actor,
        TitleBackgroundCharacterSelectOverrideCandidate activeCandidate,
        Vector3 characterPosition,
        Vector3 cameraPosition)
    {
        // 最初に成立した安定窓を確定値として保持する。長時間runの緩やかな自然回転変化を
        // 同じ窓へ累積すると、初期校正が正しくてもspreadだけが増え続けるため再採取しない。
        ...
        if (!_automaticCheck.Requested
            ...
            || !string.Equals(
                ...
                activeCandidate.Id,
                StringComparison.Ordinal)
            || !TitleBackgroundCharacterSourceProbe.TryReadCharaSelectCharacterAim(
                actor,
                out var readBackPosition,
                out var naturalRotation)
            || !TitleBackgroundCharaSelectCharacterFacing.TryDeriveCalibrationOffset(
                naturalRotation,
                readBackPosition,
                cameraPosition,
                out var derivedOffset,
                out var expectedFromGeometry))
        {
            return;
        }

        var stableSample = TitleBackgroundCharaSelectCharacterFacing.IsStableCalibrationSample(
            _characterPlacement.FacingCalibrationPreviousNaturalRotation,
            _characterPlacement.FacingCalibrationPreviousExpectedFromGeometry,
            naturalRotation,
            expectedFromGeometry);
        _characterPlacement.FacingCalibrationPreviousNaturalRotation = naturalRotation;
        _characterPlacement.FacingCalibrationPreviousExpectedFromGeometry = expectedFromGeometry;
        if (!stableSample)
        {
            _characterPlacement.FacingCalibrationRejectedTransientCount++;
            return;
        }

        if (_characterPlacement.FacingCalibrationFirstOffset.HasValue)
        {
            var offsetSpread = TitleBackgroundCharaSelectCharacterFacing.AccumulateOffsetSpread(
                _characterPlacement.FacingCalibrationFirstOffset.Value,
                _characterPlacement.FacingCalibrationMinRelativeOffset,
                _characterPlacement.FacingCalibrationMaxRelativeOffset,
                derivedOffset,
                out var minRelativeOffset,
                out var maxRelativeOffset);
            _characterPlacement.FacingCalibrationMinRelativeOffset = minRelativeOffset;
            _characterPlacement.FacingCalibrationMaxRelativeOffset = maxRelativeOffset;
            if (float.IsFinite(offsetSpread))
            {
                _characterPlacement.FacingCalibrationMaxOffsetDelta = offsetSpread;
            }
        }
        else
        {
            _characterPlacement.FacingCalibrationFirstOffset = derivedOffset;
            _characterPlacement.FacingCalibrationMinRelativeOffset = 0f;
            _characterPlacement.FacingCalibrationMaxRelativeOffset = 0f;
            _characterPlacement.FacingCalibrationMaxOffsetDelta = 0f;
        }

        _characterPlacement.FacingCalibrationCapturedDuringRun = true;
        _characterPlacement.FacingCalibrationCandidateId = activeCandidate.Id;
        _characterPlacement.FacingCalibrationDerivedOffset = derivedOffset;
        _characterPlacement.FacingCalibrationNaturalRotation = naturalRotation;
        _characterPlacement.FacingCalibrationExpectedFromGeometry = expectedFromGeometry;
        _characterPlacement.FacingCalibrationSampleCount++;
    }

    private void ApplySavedViewCharacterFacing(
        in CharaSelectResolvedActorContext actor,
        TitleBackgroundCharacterSelectOverrideCandidate activeCandidate,
        Vector3 characterPosition,
        Vector3 cameraPosition)
    {
        ...
        {
            MarkCharacterFacingInactive(IsSavedViewSuppressedByAutomaticRun()
                ? "suppressed-by-run"
                : "inactive-context");
            return;
        }
        ...
        {
            MarkCharacterFacingInactive("pose-maintain-inactive");
            return;
        }
        ...
        {
            MarkCharacterFacingInactive("saved-view-unavailable");
            return;
        }

        var calibrationCaptured =
            _configuration.TitleBackgroundCharaSelectFacingCalibrationCaptured
            && string.Equals(
                _configuration.TitleBackgroundCharaSelectFacingCalibrationCandidateId,
                activeCandidate.Id,
                StringComparison.Ordinal)
            && float.IsFinite(_configuration.TitleBackgroundCharaSelectFacingCalibrationOffset);
        var calibrationOffset = calibrationCaptured
        ...
        var yaw = TitleBackgroundCharaSelectCharacterFacing.ComputeYaw(savedView.DirH, calibrationOffset);
        if (!TitleBackgroundCharacterSourceProbe.TrySetCurrentCharacterDrawRotation(actor, yaw, out var readBackRotation))
        {
            MarkCharacterFacingInactive("draw-rotation-write-failed");
            return;
        }
        ...
        _characterPlacement.FacingSettledConsecutiveFrameCount =
            TitleBackgroundCharaSelectCharacterFacing.AdvanceFacingSettledFrameCount(
                true,
                _characterPlacement.FacingSettledConsecutiveFrameCount);
        ...
        _characterPlacement.FacingCalibrationOffset = calibrationOffset;
        _characterPlacement.FacingCalibrationSource = calibrationCaptured ? "calibrated" : "default-zero";
        if (TitleBackgroundCharaSelectCharacterFacing.TryDeriveCalibrationOffset(
                readBackRotation,
                characterPosition,
                cameraPosition,
                out var derivedOffset,
                out var expectedFromGeometry))
        {
            var geometricExpectedYaw =
                TitleBackgroundCharaSelectCharacterFacing.ComputeYaw(expectedFromGeometry, calibrationOffset);
            _characterPlacement.FacingGeometricExpectedYaw = geometricExpectedYaw;
            _characterPlacement.FacingAngularError =
                TitleBackgroundCharaSelectCharacterFacing.AngularDistance(geometricExpectedYaw, readBackRotation);
            var offsetAbsError =
                TitleBackgroundCharaSelectCharacterFacing.AngularDistance(calibrationOffset, derivedOffset);
            _characterPlacement.FacingOffsetAbsError = float.IsFinite(offsetAbsError)
                ? offsetAbsError
                : null;
            if (float.IsFinite(_characterPlacement.FacingAngularError.Value))
            {
                _characterPlacement.FacingMaxAngularError = Math.Max(
                    _characterPlacement.FacingMaxAngularError ?? 0f,
                    _characterPlacement.FacingAngularError.Value);
                _characterPlacement.FacingSettledMaxAngularError =
                    TitleBackgroundCharaSelectCharacterFacing.AccumulateSettledMaxAngularError(
                        _characterPlacement.FacingSettledConsecutiveFrameCount,
                        _characterPlacement.FacingSettledMaxAngularError,
                        _characterPlacement.FacingAngularError);
            }
        ...
        var appliedError = TitleBackgroundCharaSelectCharacterFacing.AngularDistance(yaw, readBackRotation);
        if (float.IsFinite(appliedError))
        {
            _characterPlacement.FacingMaxAppliedError = Math.Max(
                _characterPlacement.FacingMaxAppliedError ?? 0f,
                appliedError);
        }

        if (_characterPlacement.FacingPreviousReadBackRotation.HasValue)
        {
            var frameDelta = TitleBackgroundCharaSelectCharacterFacing.AngularDistance(
                _characterPlacement.FacingPreviousReadBackRotation.Value,
                readBackRotation);
            if (float.IsFinite(frameDelta))
            {
                _characterPlacement.FacingMaxFrameDelta = Math.Max(
                    _characterPlacement.FacingMaxFrameDelta ?? 0f,
                    frameDelta);
            }
        }

        _characterPlacement.FacingPreviousReadBackRotation = readBackRotation;
        _characterPlacement.FacingLastError = "none";
    }

    private void MarkCharacterFacingInactive(string reason)
    {
        _characterPlacement.FacingActive = false;
        _characterPlacement.FacingSettledConsecutiveFrameCount =
            TitleBackgroundCharaSelectCharacterFacing.AdvanceFacingSettledFrameCount(false, 0);
        _characterPlacement.FacingLastError = reason;
    }
    ...
    private void MaintainCharaSelectEnvironmentNoon()
    {
        ...
        // 既定は正午固定。FRU n4gw は正午だと lighting が白飛びするため candidate-specific な時刻を書く
        // （TitleBackgroundEnvironmentTimePolicy）。Il Mheg / Elpis は正午のまま。
        var timeCandidate = ResolveCurrentOverrideCandidate();
        var requestedDayTimeSeconds = TitleBackgroundEnvironmentTimePolicy.ResolveDayTimeSeconds(timeCandidate.Id);
        _environmentNoon.LastPolicyName = TitleBackgroundEnvironmentTimePolicy.ResolvePolicyName(timeCandidate.Id);
        _environmentNoon.LastRequestedDayTimeSeconds = requestedDayTimeSeconds;

        try
        {
            if (TitleBackgroundEnvironmentNoonWriter.TryApplyDayTimeSeconds(requestedDayTimeSeconds))
            {
                _environmentNoon.AppliedFrameCount++;
                _environmentNoon.LastStatus = "applied";
                // pre-login snapshot: 書いた直後に read-only で readback を凍結する
                // （report 生成時の post-login live 読取と混同させない）。
                var readback = TitleBackgroundEnvironmentProbe.Capture();
                if (readback.Available)
                {
                    _environmentNoon.LastPreLoginDayTimeSecondsReadback = readback.DayTimeSeconds;
                }
            }
            else
            {
        ...
        catch (Exception ex)
        {
            ...
            _log.Warning(ex, "TitleBackground environment time override failed.");
        }
    }

    // ログイン画面（Title Background）が実天候（雨など）をそのまま反映して暗い雨天スカイドーム・
    // 濡れ表現になる問題への対策。背景セッション中（pre-login + CharaSelectセッション中 + hook Ready）
    // に限り、EnvManager の天候を毎フレーム候補に応じた晴天へ上書きする。IsLoggedIn は最重要ゲート：
    ...
    private void MaintainCharaSelectEnvironmentClearSky()
    {
        ...
        var requestedWeatherId = TitleBackgroundEnvironmentWeatherPolicy.ResolveRequestedWeatherId(candidate.Id);
        _environmentClearSky.LastCandidateId = candidate.Id;
        _environmentClearSky.LastRequestedWeatherId = requestedWeatherId;

        try
        {
            var applied = requestedWeatherId == TitleBackgroundEnvironmentClearSkyWriter.FairSkiesWeatherId
                ? TitleBackgroundEnvironmentClearSkyWriter.TryApplyFairSky()
                : TitleBackgroundEnvironmentClearSkyWriter.TryApplyClearSky();

            if (applied)
            {
                _environmentClearSky.AppliedFrameCount++;
                _environmentClearSky.LastStatus = "applied";
                _environmentClearSky.LastAppliedWeatherId = requestedWeatherId;
                // pre-login snapshot: 書いた直後の readback を凍結（post-login live 読取と混同させない）。
                var readback = TitleBackgroundEnvironmentProbe.Capture();
                if (readback.Available)
                {
                    _environmentClearSky.LastPreLoginWeatherReadback = readback.ActiveWeather;
                    _environmentClearSky.PreLoginSnapshotCaptured = true;
                }
            }
            else
            {
    ...
    public bool TryCaptureCharaSelectAnchorFromCurrentCharacter(out string status)
    {
        ...
        if (!TryResolveCharaSelectActorContext(out var anchorActor)
            || !TitleBackgroundCharacterSourceProbe.TryReadCharaSelectCharacterAim(anchorActor, out var position, out var rotation))
        {
