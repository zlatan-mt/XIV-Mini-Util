...
internal static unsafe class TitleBackgroundEnvironmentNoonWriter
{
    ...
    {
        return TryApplyDayTimeSeconds(NoonDayTimeSeconds);
    }

    // candidate-specific な時刻ポリシー（TitleBackgroundEnvironmentTimePolicy）が解決した
    // エオルゼア日の秒（0..86400）を書き込む。責務は DayTimeSeconds 上書きのみ（天候・露出は触らない）。
    public static bool TryApplyDayTimeSeconds(float dayTimeSeconds)
    {
        if (!float.IsFinite(dayTimeSeconds) || dayTimeSeconds < 0f || dayTimeSeconds >= 86400f)
        {
            return false;
        }
        ...
        manager->DayTimeSeconds = dayTimeSeconds;
        return true;
    }
}
