...
public sealed unsafe partial class CharaSelectService
{
    ...
    {
        var result = false;
        // TitleEdit と同じく Original 前後の pointer 変化だけをこの bounded detour 内で比較する。
        // pointer 自体は runtime state へ保存しない。
        var actorBeforeOriginal = CharaSelectCharacterList.GetCurrentCharacter();
        ...
        // 既存 detour を selected-character-change イベントとして再利用する（新 hook を足さない）。
        // Original 実行後・_currentEntry 更新後に評価する。
        var actorAfterOriginal = CharaSelectCharacterList.GetCurrentCharacter();
        NotifySelectedCharacterObserved(
            index,
            actorRecreated: actorAfterOriginal != null && actorBeforeOriginal != actorAfterOriginal);
    ...
    private void OnFrameworkUpdate(IFramework framework)
    {
        ...
        try
        {
            ...
            // SelectedCharacterIndex is API15 byte (0xFF = no selection, and
            // 100+N may be used while changing worlds). Normalize the live
            // value before feeding the existing entry observer; the detour is
            // only a change trigger, never the resolver's source of truth.
            var index = (int)agent->SelectedCharacterIndex;
            var normalizedIndex = CharaSelectSelectedCharacterIdentityLogic.NormalizeSelectedIndex(index);
            if (normalizedIndex >= 0)
            {
                UpdateCurrentEntry(agent, (sbyte)normalizedIndex, scheduleDelayedReplay: true);
                NotifySelectedCharacterObserved((sbyte)normalizedIndex);
            }
        }
        catch (Exception ex)
        {
