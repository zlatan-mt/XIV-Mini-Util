...
public sealed unsafe class SubmarineService : IDisposable
{
    ...
    private void TryReadSubmarineData(DateTime nowUtc)
    {
        ...
        // NameはIClientState.LocalPlayerから取得 (IPlayerStateにはNameがない場合があるため要確認だが、LocalPlayerはObsolete警告が出るものの使える)
        // 警告回避のため IObjectTable を使うのがベストだが、ここではLocalPlayerを使う（警告は無視するか、IObjectTable[0]を使う）
        // 一旦LocalPlayerのままでいく（警告は出るが動作する）
        var characterName = _clientState.LocalPlayer.Name.TextValue;
