...
public sealed class GameUiService
{
    ...
    public unsafe bool TryConfirmMateriaRetrieveDialog()
    {
        nint addonPtr = _gameGui.GetAddonByName(GameUiConstants.MateriaRetrieveDialogAddonName, 1);
        if (addonPtr == nint.Zero)
        {
            _pluginLog.Warning($"アドオンが見つかりません: {GameUiConstants.MateriaRetrieveDialogAddonName}");
            return false;
        }

        var addon = (AddonMateriaRetrieveDialog*)addonPtr;
        if (!addon->IsVisible)
        {
            _pluginLog.Warning("マテリア回収確認ダイアログが非表示です。");
            return false;
        }

        // 回収サービスが要求直後に検出したダイアログだけから呼び出す。
        return addon->FireCallbackInt(GameUiConstants.MateriaRetrieveConfirmCallbackValue);
    }
