...
- `docs` 配下に設計、配布、実装メモ、旧版アーカイブが混在している
...
この区別により、仕様策定中の文書と現状把握資料を混同しないようにする。
...
- `projects/XIV-Mini-Util`
  - 本体コード
- `projects/*` の外部参照用コード
  - 将来的に `external/` または `vendor/` への分離を検討

外部参照プロジェクトの移動は影響が大きいため、別フェーズで行う。
...
完了条件:
- `docs` 直下には分類ディレクトリと `archive` のみがある
...
- `.kiro/specs` は仕様管理のため、`docs` 側の都合で移動しない
- 外部参照プロジェクトは安易に動かさない
- 旧版文書は削除せず、必要なら `archive` に逃がす
