...
### 16.1 hook は段階的に有効化する

いきなり `CreateScene` / `FixOn` / `LobbyUpdate` をすべて差し替えない。

推奨順:

1. resolver と status 表示だけを実装する。
2. `LobbyUpdate` hook を logging/status 目的で有効化する。
3. `CreateScene` hook を logging/status 目的で有効化する。
4. valid `TerritoryPath` のときだけ `CreateScene` path override を有効化する。
5. 最後に `FixOn` camera/focus/FOV override を有効化する。

override disabled の間は、hook が作成できても差し替え処理を行わない。

### 16.2 `CreateScene` の title 判定は fail-closed にする

`CreateScene` が title scene かどうかを `_lastLobbyUpdateMapId` だけに依存しない。

可能なら以下を併用する。

- `CurrentLobbyMap`
- `LobbyUpdate` で記録した直近 `mapId`
- `CreateScene` に渡ってきた元 path
- hook 呼び出し順ログ

判定できない場合は override しない。  
迷ったら通常処理へ戻す。

### 16.3 `TerritoryPath` の正規化を helper に集約する

`TitleBackgroundTerritoryPath` は以下を正規形とする。

```text
ffxiv/.../level/...
```

UI入力で以下が入った場合の扱いを helper に集約する。
...
validation 用の実ファイルパス生成も1箇所に集約する。

例:

```csharp
private static string NormalizeTerritoryPathInput(string input);
private static string BuildLvbPath(string normalizedTerritoryPath);
private bool ValidateTerritoryPath(string normalizedTerritoryPath, out string errorMessage);
```

invalid path の場合:

- hook 内で差し替えない。
- UI に validation error を表示する。
- 設定値は勝手に消さない。

### 16.4 persisted enabled の起動リスクを抑える

plugin 起動時に `TitleBackgroundOverrideEnabled == true` でも、resolver/hook 初期化に失敗した場合は `TitleBackground` 機能だけ disable する。

ただし以下のユーザー設定値は消さない。

- `TitleBackgroundTerritoryPath`
- camera position
- focus position
- FOV
- BGM/weather/time の将来用設定

status に失敗理由を残す。

### 16.5 実装結果docに実機ログ観点を残す

`docs/notes/title-background-implementation-result.md` には、可能な範囲で以下を記録する。
...
- invalid path 時の挙動
- plugin unload 時の hook dispose 結果

キャラクター名、ワールド名、個人名、ローカルパスなどは伏せる。
