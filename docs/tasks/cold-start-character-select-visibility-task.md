...
- same-tick placement-retention drift as scalar distances only (`retention.*`): terminal comparability, last-valid / max drift, comparable vs not-comparable sample counts, current-interval vs closed-interval stats kept separate (`retention.currentInterval*` / `retention.lastClosedInterval*` / `retention.closedIntervalsMaxDriftMeters`), closed-interval count, `retention.awaitingNewConfirmedPlacement`, `retention.epsilonMeters` = the placement write path's own readback epsilon. The interval closes when a new confirmed placement lands OR when the currently observed (validly resolved) actor / scene generation leaves the interval's baseline target; after an observed-target departure comparison stays suppressed until a new confirmed placement, so an A→B→A actor bounce (or a scene change) with no re-placement never keeps a stale interval alive or emits a false drift-recovered event. A read failure alone is not a target change;
- bounded event checkpoints (`coldStart.checkpoint[i]=…`, cap 24 + `checkpoints.dropped`) for identity change (with run-local anonymous actor slots and component-change booleans), placement apply/write attempts, drift exceed/recover, and retention-interval close (with reason, max / last-valid drift and comparable-sample count);
